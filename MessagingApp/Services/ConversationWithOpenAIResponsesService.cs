...
public class ConversationWithOpenAIResponsesService(OpenAIClient openAIClient,
        IOptions<AssistantOptions> assistantOptions,
        ILogger<ConversationWithOpenAIResponsesService> logger) : IConversationService
{
    private readonly List<Conversation> _conversations = new();
    private readonly object _lock = new();

    private readonly OpenAIResponseClient _responsesClient
        = openAIClient.GetOpenAIResponseClient(assistantOptions.Value.ModelName);
        ...
        else
        {
            try
            {
                ...
                else
                {
                    ...
                    logger.LogWarning("OpenAI Responses API returned incomplete response or empty text.");
                }
            }
            catch (Exception ex)
            {
                logger.LogError(ex, "Error while calling OpenAI Responses API.");
            }
        }
